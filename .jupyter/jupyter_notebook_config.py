...
# template ansible run date: 2018-04-05 14:42:51.625732
# /home/tcoprea/.jupyter/jupyter_notebook_config.py
...
def post_save(model, os_path, contents_manager):
    """post-save hook for converting notebooks to scripts"""
    if model['type'] != 'notebook':
        return # only do this for notebooks
    current_directory, fname = os.path.split(os_path)
    try:
        # nodejs notebooks throw an error here
        check_call(
            ['/opt/conda/bin/jupyter', 'nbconvert', '--to', 'script', fname], cwd=current_directory)
    except subprocess.CalledProcessError as e:
        print("Cannot convert the '{}' notebook to a script.".format(fname))

c = get_config()
c.FileContentsManager.post_save_hook = post_save
c.NotebookApp.notebook_dir = os.path.join(os.path.expanduser('~'), "notebooks")

#------------------------------------------------------------------------------
# Application(SingletonConfigurable) configuratio
#------------------------------------------------------------------------------
